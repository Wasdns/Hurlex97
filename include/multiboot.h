...
typedef struct multiboot_t {
    ...
    低端内存的首地址是 0 ，高端内存的首地址是 1M 。低端内存的最大可能值是 640K 
...
base_addr_high 是高 32 位;
启动地址总共有 64 位.

length_low 是内存区域大小的低 32 位;
length_high 是内存区域大小的高 32 位;
总共是 64 位.

type 是相应地址区间的类型:
1 代表可用 RAM，所有其它的值代表保留区域 */

typedef struct mmap_entry_t {
    uint32_t size;                      // size 不含其自身变量(size)的大小
